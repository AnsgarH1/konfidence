...
func TestListForProject_ReturnsLandscapes(t *testing.T) {
	...
	k8s := fakeClient(l1, l2)

	result, err := landscape.ListForProject(context.Background(), k8s, "kden-p-my-project")
	if err != nil {
...
func TestListForProject_EmptyNamespace(t *testing.T) {
	...
	result, err := landscape.ListForProject(context.Background(), k8s, "kden-p-empty")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(result) != 0 {
...
func TestListForProject_OnlyReturnsNamespacedLandscapes(t *testing.T) {
	...
	k8s := fakeClient(lA, lB)

	result, err := landscape.ListForProject(context.Background(), k8s, "kden-p-project-a")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(result) != 1 {
