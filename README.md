...
The preview server is intended for checking the root page. To exercise production SPA fallback behavior, serve the build through the API with `go run ./cmd/api --ui-dir=apps/konfidence-ui/build`.

Production releases include the static SPA in the API image. The API serves the dashboard and `/api/v1` from the same origin; set `API_UI_DIR` when running an API binary with an external dashboard build directory.
